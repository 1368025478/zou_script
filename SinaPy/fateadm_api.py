...
def TestFunc(path):
    ...
    file_name       = path
    #
    # icon = requests.get(path)
    # iconData = icon.read()
    imges_path = os.getcwd()

    tm = str(int(time.time()))

    img_save_path = imges_path+'/code'+tm+'.png'

    urllib.urlretrieve(path, img_save_path)
    ...
    rsp             = api.PredictFromFile(pred_type, img_save_path) # 返回识别结果的详细信息
